...
if os.getenv("DOCKER_FRONTEND", "false").lower() == "true":
    API_BASE = "http://fastapi-backend:8000"  # Docker hálózaton a backend konténer neve
else:
    # Lokális futtatás
            ...
            try:
                ...
                if r.status_code == 200:
                   ...
                   # st.experimental_rerun()
                    st.experimental_set_query_params()  # opcionális, ha URL paramétert akarsz frissíteni
                else:
                    try:
            ...
            try:
                ...
                if r.status_code in (200, 201):
                    ...
                    try:
                        ...
                        err_detail = r.text
                    st.error(f"Hiba történt: {r.status_code} — {err_detail}")
